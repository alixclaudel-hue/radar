...
**La délégation (`scripts/ai_broker.py`) passe AVANT Claude sur les logs et sorties volumineuses.** Avant
d'ingérer des logs Docker bruts dans le contexte :

- **Logs Docker (>50 lignes)** : `docker logs <conteneur> --tail 200 | python3 scripts/ai_broker.py --mode diag --stdin`
