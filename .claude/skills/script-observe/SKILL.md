...
Lis avec `delegate` (mode context) `scripts/loop/registry.json` (dans `~/radar`, en lecture) : l'entrée du
...
- Aucun job lourd en cours (pour cela lis avec `delegate` (mode context) `*.status.json`) — surtout si la collecte consomme
...
Utilise `delegate` (modes code et pr) pour pousser et journaliser
```
cd ~/radar-diag
