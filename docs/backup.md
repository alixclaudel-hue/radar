...
- `scripts/backup.sh` : `tar` de `/data` (hors `data/backups/` et
  `data/shared/discogs_dump.sqlite3`, référentiel Discogs reconstructible depuis
  le dump mensuel, cf. `discogs_dump.py` — pas une donnée utilisateur) →
  chiffrement **AES-256** (openssl, PBKDF2 200k) en flux →
  `/data/backups/data-<ts>.tgz.enc`.
- Rétention : 14 fichiers (les plus récents).
- Le log (`backup.log`) inclut désormais un relevé de taille par sous-dossier de
  `/data`, pour repérer une dérive sans avoir à se reconnecter en SSH.
